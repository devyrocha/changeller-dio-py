...
def insert_users_data(session):
    yuri = Usuario(
        nome='yuri',
        nome_completo='Yuri Rocha',
        endereco=[Endereco(endereco_email='yuri@example.com')]
    )

    fulano = Usuario(
        nome='fulano',
        nome_completo='Fulano de Tal',
        endereco=[Endereco(endereco_email='fulano@example.com'),
                 Endereco(endereco_email='fulano2@example.com')]
    )

    jose = Usuario(nome='jose', nome_completo='José da Silva')

    session.add_all([yuri, fulano, jose])
    session.commit()


def retrieve_users_by_name(session):
    stmt = session.query(Usuario).filter(Usuario.nome.in_(["yuri", 'fulano']))
